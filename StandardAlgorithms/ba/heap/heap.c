...
void push_heap(value_type *a, size_type n) /*@ ghost(size_type n2) */ // Everything is a heap, except for the very last element
{
  ...
  if (a[child] > a[parent])
  {
    ...
    // a is now a heap but one element is different from the multiset
    ...
    */
    ...
    //@ assert 0 == child || temp <= a[HeapParent(child)];
...
*/
void pop_head(value_type *a, size_type n) // Move the head to the back and fix the heap 0..n-1
...
*/
void heapify(value_type *a, size_type n) /*@ ghost(size_type n2) */
